...
// testQueuePush tests K2hashQueue.Push method.
...
func testQPushArgs(d qdata, t *testing.T) {
	k, err := k2hash.NewK2hash("/tmp/test.k2h")
	if err != nil {
		t.Errorf("k2hash.NewK2hash(/tmp/test.k2h) return err %v", err)
	}
	defer k.Close()
	q, err := k2hash.NewQueue(k)
	if err != nil {
		t.Errorf("k2hash.NewK2hashQueue(%v) return err %v", k, err)
	}
	defer q.Free()
	// 1. push
	if ok, err := q.Push(string(d.v)); !ok {
		t.Errorf("K2hashQueue.Push(%v) return false. wants true. err %v", string(d.k), err)
	}
	// 2. count
	if c, _ := q.Count(); c != 1 {
		t.Errorf("K2hashQueue.Count() return not 1. wants 1")
	}
	// 3. pop
	if s, err := q.Pop(); s != string(d.v) {
		t.Errorf("K2hashQueue.Pop( return %v. wants true. err %v", string(d.k), err)
	}
